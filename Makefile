...
docker-build: tidy vendor
	docker buildx build --platform linux/arm64,linux/amd64 -t public.ecr.aws/p3k6k6h3/event-hub-poc:$(DOCKER_TAG) . --load

.PHONY: docker-push
docker-push: tidy vendor docker-login
	docker buildx build --platform linux/arm64,linux/amd64 -t public.ecr.aws/p3k6k6h3/event-hub-poc:$(DOCKER_TAG) . --push

.PHONY: build
build: tidy vendor
	CGO_ENABLED=0 go build -mod=vendor -ldflags="-w -s" -o event-hub-poc .
